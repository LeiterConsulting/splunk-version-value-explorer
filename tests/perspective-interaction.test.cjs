...
test('perspective changes redraw in place, preserve route and scroll, and explain scope',()=>{
 const nodes=[];function el(tag='div'){const n={tagName:tag.toUpperCase(),children:[],attrs:{},events:{},classList:{add(){},remove(){}},append(...v){this.children.push(...v)},appendChild(v){this.children.push(v)},replaceWith(){},setAttribute(k,v){this.attrs[k]=v},addEventListener(k,fn){this.events[k]=fn},getBoundingClientRect(){return {height:100}},querySelector(s){if(s==='select')return this.select??=el('select');return null}};nodes.push(n);return n;}
