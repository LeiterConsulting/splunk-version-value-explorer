...
Preserve [decision support and snapshots](decision-support.md): four-part summaries, actionable evidence-linked questions, selection-specific retained change history, and script-free downloadable report snapshots. Record corrected guidance and resolved conflicts in content-update history, retaining entries after badge expiry. Regenerate the content revision after release/source synchronization and run its --check gate before every publication, including operational outcomes. A revision identifies shipped content; external source documents remain live.
