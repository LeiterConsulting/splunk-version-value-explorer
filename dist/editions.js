(function () {
  ...
  const evidence=c=>window.VersionCompassEvidence?.html({claim:c.name+': '+c.desc,scope:'ES '+data.release+' · Essentials / Premier · '+c.lane,urls:c.src.map(k=>data.sources[k].u),qualification:c.flag||'Edition inclusion is separate from deployment, activation and pairing requirements.'})||'';
  ...
  function report() { return `<article class="edition-report" aria-label="Printable ES editions report">
    ...
    <table class="report-matrix"><colgroup><col style="width:54%"><col style="width:23%"><col style="width:23%"></colgroup><thead><tr><th scope="col">Capability and requirements</th><th scope="col">Essentials</th><th scope="col">Premier</th></tr></thead><tbody>${data.capabilities.map(c=>`<tr><td><h3>${esc(c.name)}${window.VersionCompassUpdates?.html("edition:"+c.id)||""}</h3><p>${esc(c.desc)}</p>${c.flag ? '<p class="report-row-note">'+esc(c.flag)+'</p>' : ''}${refs(c.src)}${evidence(c)}</td><td>${reportCell(c.ess)}</td><td>${reportCell(c.prem)}</td></tr>`).join('')}</tbody></table></section>
  ...
  document.querySelector('main').innerHTML = `
    ...
    <div class="edition-body" id="edition-comparison">
      ...
      <section class="edition-matrix-section" aria-labelledby="edition-matrix-title">
        ...
        <div id="edition-rows">${data.capabilities.map(c => `<article class="edition-row" data-id="${esc(c.id)}"><div class="edition-row-grid"><div><h3>${esc(c.name)}${window.VersionCompassUpdates?.html("edition:"+c.id)||""}</h3><small>${esc(c.lane)}${c.tag ? ' · '+({new:'New in 8.7',updated:'Updated in 8.7',conf:'September announcement'}[c.tag]) : ''}</small><p class="edition-description">${esc(c.desc)}</p>${sources(c.src)}${evidence(c)}<div id="env-cap-${esc(c.id)}"></div></div><div class="edition-cell"><span class="edition-mobile-label">Essentials</span>${status(c.ess)}</div><div class="edition-cell"><span class="edition-mobile-label">Premier</span>${status(c.prem)}</div></div>${c.ess.v === 'review' ? '<p class="edition-qualification"><strong>'+ (c.id==='connector-builder'?'Edition conflict: ':'Scope question: ')+'</strong><a href="#conflict-'+esc(c.id)+'">Compare the two source statements and what remains unresolved ↓</a></p>' : ''}${c.flag && c.ess.v !== 'review' ? '<details><summary>Additional qualification</summary><p>'+esc(c.flag)+'</p>'+sources(c.src)+'</details>' : ''}</article>`).join('')}</div>
