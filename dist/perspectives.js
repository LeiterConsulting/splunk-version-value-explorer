/* Unlisted presentation experiment; all evidence comes from the shared engine. */
(function(){
'use strict';
...
function preserve(value){const u=new URL(value,location.href);if(u.origin===location.origin){u.searchParams.set('lab','perspectives');u.searchParams.set('perspective',role);}return u.pathname+u.search+u.hash;}
...
document.documentElement.classList.add('perspective-lab');
...
label.querySelector('select').addEventListener('change',e=>{const u=new URL(location.href);u.searchParams.set('lab','perspectives');u.searchParams.set('perspective',e.target.value);location.assign(u.href)});
const notice=document.createElement('p');notice.className='perspective-notice';notice.textContent='Perspective preview · Cloud guidance is prioritized for your view. All matching evidence remains available.';header.after(notice);
