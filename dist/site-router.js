/* Only an explicit, unambiguous URL switch loads the unlisted preview. */
(function () {
  'use strict';
  const params = new URLSearchParams(window.location.search);
  const preview = params.getAll('preview').length === 1 && params.get('preview') === 'es-editions';
  ...
  if (preview) {
    ...
    document.querySelector('main').innerHTML = '<section class="hero"><p class="dek" role="status">Loading ES editions preview…</p></section>';
